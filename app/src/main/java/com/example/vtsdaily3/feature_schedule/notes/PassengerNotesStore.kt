...
object PassengerNotesStore {
    ...
    private fun loadAll(context: Context): MutableMap<String, PassengerResidenceNote> {
        ...
        return try {
            ...
            gson.fromJson<MutableMap<String, PassengerResidenceNote>>(text, type) ?: mutableMapOf()
        } catch (_: Exception) {
            mutableMapOf()
        }
    }

    private fun saveAll(context: Context, records: Map<String, PassengerResidenceNote>) {
        try {
    ...
    fun get(context: Context, recordKey: String): PassengerResidenceNote? {
        return loadAll(context)[recordKey]
    }

    fun put(context: Context, note: PassengerResidenceNote) {
